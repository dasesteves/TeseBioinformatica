...
\begin{itemize}
    \item As-is architecture diagram (SCMVV): Introduction (Section~\ref{sec:as_is_architecture}); Figure label \path{fig:as_is_architecture_scmvv}.
    \item Current medication process swimlane: Introduction (Section~\ref{sec:current_process_org}); Figure label \path{fig:as_is_swimlane_scmvv}.
    \item Baseline artefacts (legacy screens, anonymized extracts): Results (Baseline sections), with table/figure labels \path{fig:baseline_*} or \path{tab:baseline_*}.
    ...
    \item User acceptance artifacts (SUS summary, interview quotes): Results (Stakeholder Feedback), tables/figures \path{tab:sus_*}, \path{fig:feedback_*}.
